...
import com.calopsite.demo.services.NameMutations;
...
public class MutationResource {
    @Autowired
    private NameMutations nameMutations;

    @GetMapping
    public ResponseEntity<List<Mutation>> findAll(){
        List<Mutation> list = nameMutations.findAll();
        return ResponseEntity.ok().body(list);
    }
    @GetMapping(value = "/{id}")
    public ResponseEntity<Mutation> findByID(@PathVariable Long id){
        Mutation obj = nameMutations.findByID(id);
